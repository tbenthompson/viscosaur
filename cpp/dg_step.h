                    ...
                    ++cell)
            {
                ...
                for (unsigned int v=0; v<pd.strs_matrix_free.n_components_filled(cell); ++v)
                {
                            ...
                            ++face)
                    {
                        ...
                        {
                            for(int nvf; nvf < neighbor_vel_face.size(); nvf++)
                            {
                                neighbor_vel_face[nvf] = 0.0;
                            }
                        } 
                        else
                        {
                        ...
                        {
                            flux = d->mu[0] * (this_vel_face[q] + neighbor_vel_face[q]) +
                                (d->C / 2.0) * (this_vel_face[q] - neighbor_vel_face[q]);
                            for (unsigned int i=0; i < dofs_per_cell; ++i)
                            {
                                component_i = pd.strs_fe.system_to_component_index(i).first;
                                dst(local_dof_indices[i]) += flux * 
                                    strs_face_values[strs_access].value(i, q)[component_i] *
                                    strs_face_values.normal_vector(q)[component_i] * 
